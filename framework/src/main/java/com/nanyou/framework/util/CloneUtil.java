...
public class CloneUtil {  
     ...
     * 判断该类型对象是否无�?���?
     * @param c 指定类型 
     * @return 如果不需要复制则返回真，否则返回�?
     */  
    private static boolean isNeedlessClone(Class c){  
        if(c.isPrimitive()){//基本类型  
            return true;  
        }  
        for(Class tmp:needlessCloneClasses){//是否在无�?��制类型数组里  
            if(c.equals(tmp)){  
                return true;  
            }  
        }  
     ...
     * 复制对象数据 
     * @param value 原始对象 
     * @param level 复制深度。小�?为无限深度，即将深入到最基本类型和Object类级别的数据复制�?
     * 大于0则按照其值复制到指定深度的数据，等于0则直接返回对象本身�?不进行任何复制行为�? 
     * @return 返回复制后的对象 
     * @throws IllegalAccessException 
    ...
    public static Object clone(Object value,int level) throws IllegalAccessException, InstantiationException, InvocationTargetException, NoSuchMethodException{  
        ...
        if(value instanceof Collection){//复制新的集合  
            ...
            if(tmp==null){//无法创建新实例则返回对象本身，没有克�? 
                return value;  
            }  
